...
def moving_average(df, close_col_name, n):
    ...
    """
    MA = pd.Series(
        df[close_col_name].rolling(n, min_periods=n).mean(), name="MA_" + str(n)
    )
    df = df.join(MA)
    return df


def exponential_moving_average(df, close_col_name, n):
    """
    ...
    """
    EMA = pd.Series(
        df[close_col_name].ewm(span=n, min_periods=n).mean(), name="EMA_" + str(n)
    )
    df = df.join(EMA)
    return df


def momentum(df, close_col_name, n):
    """
    ...
    :return: pandas.DataFrame
    """
    M = pd.Series(df[close_col_name].diff(n), name="Momentum_" + str(n))
    df = df.join(M)
    return df


def bollinger_bands(df, close_col_name, n):
    """
...
def rate_of_change(df, close_col_name, n):
    """
...
"""
    Additional unused technical indicators
"""
